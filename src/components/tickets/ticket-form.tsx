  ...
  const onSubmit: SubmitHandler<TicketFormValues> = async (data) => {
    ...
    if (result?.error) {
      toast({
        variant: "destructive",
        title: "Error",
        description: typeof result.error === 'string' ? result.error : "An error occurred.",
      });
    } else {
      toast({
        title: "Success",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 py-4">
      ...
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        <div className="col-span-3">
          <Select {...register("diet")} onValueChange={(value) => reset({ ...watch(), diet: value })} value={dietValue}>
            <SelectTrigger><SelectValue placeholder="Select a diet" /></SelectTrigger>
            <SelectContent>
          ...
          {errors.diet && <p className="text-sm text-destructive mt-1">{errors.diet.message}</p>}
        </div>
      </div>
      <div className="grid grid-cols-4 items-center gap-4">
        ...
        <div className="col-span-3">
          <Select {...register("mealTime")} onValueChange={(value) => reset({ ...watch(), mealTime: value })} value={mealTimeValue}>
          ...
          {errors.mealTime && <p className="text-sm text-destructive mt-1">{errors.mealTime.message}</p>}
        </div>
      </div>
